...
struct GPUVertexBuffer
{
	...
	bgfx::TransientVertexBuffer m_PosBufferHandle;
	bgfx::TransientVertexBuffer m_UVBufferHandle;
	bgfx::TransientVertexBuffer m_ColorBufferHandle;
#else
...
{
#if VG_CONFIG_USE_TRANSIENT_BUFFERS
	bgfx::TransientIndexBuffer m_bgfxHandle;
#else
...
static bool isPathCulled(Context* ctx, const float* pathVertices, float margin);

// The maximum length of an extrusion vector generated by the stroker, relative to the extrusion
// distance (see calcExtrusionVector() in stroker.cpp: |d01 - d12| <= 2 and |cross| > 0.01).
...
void end(Context* ctx)
{
	...
	for (uint32_t iVB = ctx->m_FirstVertexBufferID; iVB < numVertexBuffers; ++iVB) {
		...
		bgfx::allocTransientVertexBuffer(&gpuvb->m_UVBufferHandle, vb->m_Count, ctx->m_UVVertexDecl);
		bx::memCopy(gpuvb->m_UVBufferHandle.data, vb->m_UV, sizeof(uv_t) * 2 * vb->m_Count);
#if VG_CONFIG_UV_INT16
		releaseVertexBufferData_UV(ctx, vb->m_UV);
#else
		releaseVertexBufferData_Vec2(ctx, vb->m_UV);
		...
		bgfx::allocTransientVertexBuffer(&gpuvb->m_ColorBufferHandle, vb->m_Count, ctx->m_ColorVertexDecl);
...
{
	// NOTE: The scissor rect is expanded by 1 pixel on each side because it's truncated to integers
	// when submitted (see allocDrawCommand()).
	const float* scissor = state->m_ScissorRect;
	return maxx < scissor[0] - 1.0f
		|| maxy < scissor[1] - 1.0f
		|| minx > scissor[0] + scissor[2] + 1.0f
		|| miny > scissor[1] + scissor[3] + 1.0f;
}

// Returns true if the geometry generated for the current (transformed) path cannot be visible
// because it's completely outside the scissor rect. 'margin' is the maximum distance generated
// geometry (stroke width, joins, AA fringe) can extend away from the path vertices.
static bool isPathCulled(Context* ctx, const float* pathVertices, float margin)
{
	const State* state = getState(ctx);
	const float* scissor = state->m_ScissorRect;
	...
	if (!ctx->m_PathBoundsValid) {
		const uint32_t numVertices = pathGetNumVertices(ctx->m_Path);
		if (numVertices == 0) {
			return false;
		}

		float minx = pathVertices[0], miny = pathVertices[1];
		float maxx = minx, maxy = miny;
		for (uint32_t i = 1; i < numVertices; ++i) {
			const float x = pathVertices[i * 2 + 0];
			const float y = pathVertices[i * 2 + 1];
			minx = bx::min<float>(minx, x);
			miny = bx::min<float>(miny, y);
			maxx = bx::max<float>(maxx, x);
			maxy = bx::max<float>(maxy, y);
		}

		ctx->m_PathBounds[0] = minx;
		ctx->m_PathBounds[1] = miny;
		ctx->m_PathBounds[2] = maxx;
		ctx->m_PathBounds[3] = maxy;
		ctx->m_PathBoundsValid = true;
	}

	// NOTE: NaN bounds compare false everywhere, so they are never culled.
	const float* bounds = ctx->m_PathBounds;
	return isRectOutsideScissor(state, bounds[0] - margin, bounds[1] - margin, bounds[2] + margin, bounds[3] + margin);
}
...
static void renderTextQuads(Context* ctx, const FONSquad* quads, uint32_t numQuads, Color color, float tx, float ty)
{
	...
	// Skip text which is completely outside the scissor rect.
	{
		const float* scissor = state->m_ScissorRect;
		if (scissor[2] < 1.0f || scissor[3] < 1.0f) {
			return;
		...
		float qminx = quads[0].x0, qminy = quads[0].y0, qmaxx = quads[0].x1, qmaxy = quads[0].y1;
		for (uint32_t i = 1; i < numQuads; ++i) {
			qminx = bx::min<float>(qminx, quads[i].x0);
			qminy = bx::min<float>(qminy, quads[i].y0);
			qmaxx = bx::max<float>(qmaxx, quads[i].x1);
			qmaxy = bx::max<float>(qmaxy, quads[i].y1);
		}

		float corners[8];
		vgutil::transformPos2D(qminx, qminy, mtx, &corners[0]);
		vgutil::transformPos2D(qmaxx, qminy, mtx, &corners[2]);
		vgutil::transformPos2D(qmaxx, qmaxy, mtx, &corners[4]);
		vgutil::transformPos2D(qminx, qmaxy, mtx, &corners[6]);
		const float minx = bx::min<float>(bx::min<float>(corners[0], corners[2]), bx::min<float>(corners[4], corners[6]));
		const float miny = bx::min<float>(bx::min<float>(corners[1], corners[3]), bx::min<float>(corners[5], corners[7]));
		const float maxx = bx::max<float>(bx::max<float>(corners[0], corners[2]), bx::max<float>(corners[4], corners[6]));
		const float maxy = bx::max<float>(bx::max<float>(corners[1], corners[3]), bx::max<float>(corners[5], corners[7]));
		if (isRectOutsideScissor(state, minx, miny, maxx, maxy)) {
			return;
		}
	}
